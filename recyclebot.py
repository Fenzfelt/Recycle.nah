...
trash = ['botol plastik','sampah kaca','kertas','logam','karton','plastik','kardus','kaleng','majalah',
         'tutup botol','botol kaca','koran',]

@bot.command()
async def ceksampah(ctx):
    await ctx.send('Sampah apa yg mau anda cek?')
    message = await bot.wait_for("message", check=lambda m: m.author == ctx.author and m.channel == ctx.channel)
    message = str(message.content)

    #Checking
    if message.lower() in trash:
        await ctx.send('sampah tersebut harus di daur ulang, berikut tips daur ulang')
        await ctx.send('https://waste4change.com/blog/daur-ulang-sampah/')
    else:
        await ctx.send('sampah tersebut bisa dimusnahkan/dibuang dengan bijak')
        await ctx.send('https://www.dbs.com/spark/index/id_id/site/articles/livemorekind/2022-jenis-sampah-dan-tempat-membuangnya.html')

bot.run('WHITE NIGHT ma fav song :)')
